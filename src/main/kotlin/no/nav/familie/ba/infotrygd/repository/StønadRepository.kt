...
interface StønadRepository : JpaRepository<Stønad, Long> {
    ...
    @Query(
        """SELECT new no.nav.familie.ba.infotrygd.model.dl1.TrunkertStønad(s.id, s.personKey, s.fnr, s.sakNr, s.saksblokk, s.status, s.region, s.virkningFom, s.opphørtFom, s.iverksattFom, s.antallBarn, p.pensjonstrygdet) FROM Stønad s
           INNER JOIN Person p
                   ON (s.personKey = p.personKey and
                       s.region = p.region)
        WHERE p.fnr = :fnr
    ...
    fun findTrunkertStønadMedUtbetalingÅrByFnr(
        fnr: FoedselsNr,
        år: Int,
    ): List<TrunkertStønad>
    ...
    fun findStønadMedUtbetalingByÅrAndStatusKoder(
        år: Int,
        vararg statusKoder: String,
    ): List<TrunkertStønad>
    ...
    fun findStønadByÅrAndStatusKoderAndFnr(
        fnr: FoedselsNr,
        år: Int,
        vararg statusKoder: String,
    ): List<Stønad>
