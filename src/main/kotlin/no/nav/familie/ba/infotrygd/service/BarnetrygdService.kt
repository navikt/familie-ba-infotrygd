...
class BarnetrygdService(
    ...
    fun findStønadByBrukerFnr(
        brukerFnr: List<FoedselsNr>,
        historikk: Boolean? = false,
    ): List<StønadDto> =
        if (brukerFnr.isEmpty()) {
            emptyList()
        } else {
            when (historikk) {
    ...
    fun findSakerByBarnFnr(barnFnr: List<FoedselsNr>): List<SakDto> =
        if (barnFnr.isEmpty()) {
            ...
            sakRepository
                .findBarnetrygdsakerByBarnFnr(barnFnr)
                .distinct()
    ...
    fun hentDelytelseOgKonverterTilDto(stønad: Stønad): StønadDto =
        StønadDto(
            ...
            delytelse =
                stønad.fnr?.let {
                    vedtakRepository
                        .hentVedtak(it.asString, stønad.sakNr.trim().toLong(), stønad.saksblokk)
                        .sortedBy { it.vedtakId }
                        .lastOrNull()
                        ?.delytelse
                        ?.sortedBy { it.id.linjeId }
                        ?.map { it.toDelytelseDto() }
                } ?: emptyList(),
    ...
    fun konverterTilDto(sak: Sak): SakDto {
        ...
        return SakDto(
            ...
            stønad =
                stonadRepository
                    .findStønadBySak(sak)
                    .minByOrNull { it.virkningFom.toInt() }
    ...
    fun tellAntallÅpneSaker(
        ...
        if (!barn.isNullOrEmpty()) {
            val personerViaBarn =
                barnRepository
                    .findBarnByFnrList(barn.map { FoedselsNr(it) })
    ...
    fun finnBarnetrygdForPensjon(
        ...
        val barnetrygdStønader =
            stonadRepository
                .findTrunkertStønadMedUtbetalingÅrByFnr(brukerFnr, fraDato.year)
        ...
        val barnetrygdFraRelaterteSaker =
            barnRepository
                .findBarnByFnrList(perioder.map { FoedselsNr(it.personIdent) })
                ...
                .mapNotNull { relatertBrukerFnr ->
                    BarnetrygdTilPensjon(
                        ...
                        barnetrygdPerioder =
                            stonadRepository
                                .findTrunkertStønadMedUtbetalingÅrByFnr(relatertBrukerFnr, fraDato.year)
                                .filter { erRelevantStønadForPensjon(it) }
                                .filter { filtrerStønaderSomErFeilregistrert(it) }
    ...
    fun finnUtvidetBarnetrygdBisys(
        ...
        val utvidetBarnetrygdStønader =
            stonadRepository
                .findStønadByFnr(listOf(brukerFnr))
                .filter { it.antallBarn > 0 }
        ...
        return InfotrygdUtvidetBarnetrygdResponse(
            perioder.filter {
    ...
    fun finnPersonerBarnetrygdPensjon(år: String): List<FoedselsNr> {
        ...
        return stønaderMedAktuelleKoder
            .filter {
    ...
    fun finnUtvidetBarnetrygdBeløpNårStønadIkkeHarStatus0(utbetaling: Utbetaling): Double =
        if (utbetaling.fom()!!.isAfter(YearMonth.of(2019, 2))) {
    ...
    fun finnSisteVedtakPåPerson(personKey: Long): YearMonth =
        stonadRepository.findSenesteIverksattFomByPersonKey(personKey).let {
            DatoUtils.seqDatoTilYearMonth(it)!!
        }
    ...
    fun harSendtBrevForrigeMåned(
        personidenter: List<FoedselsNr>,
        brevkoder: List<String>,
    ): List<Hendelse> {
    ...
    private fun skalFiltreresPåDato(
        fraDato: YearMonth,
        fom: YearMonth,
        tom: YearMonth?,
    ): Boolean {
    ...
    private fun erUtvidetBarnetrygd(stønad: TrunkertStønad): Boolean {
        return when (stønad.status.toLong()) {
            ...
            // Sykt barn (Ikke lenger i bruk, kan forekomme i gamle tilfeller),
            else -> {
                false
            }
        }
    ...
    private fun erRelevantStønadForPensjon(stønad: TrunkertStønad): Boolean {
        return when (stønad.status.toLong()) {
            ...
            // Ordinær barnetrygd - Maskinell beregning
            2L -> {
                true
            }

            // Utvidet barnetrygd - Maskinell beregning.
            3L -> {
                false
            }
            ...
            // Ordinær barnetrygd - Institusjon
            else -> {
                false
            }
        }
    ...
    private fun konverterTilDtoForPensjon(
        ...
        barnetrygdStønader.forEach { stønad ->
            val utbetalinger =
                utbetalingRepository.hentUtbetalingerByStønad(stønad).filterNot {
                    it.erSmåbarnstillegg()
                }
            val barna =
                barnRepository.findBarnByPersonkey(stønad.personKey, true).filter {
            ...
            allePerioder.addAll(
                utbetalinger.flatMap { utbetaling ->
                    ...
                    barna
                        .filter { it.barnetrygdTom()?.isSameOrAfter(utbetaling.fom()!!) ?: true }
                        .map { barn ->
                            ...
                            BarnetrygdPeriode(
                                ...
                                stønadTom =
                                    minOf(utbetalingTom, barnetsOpphørsdato).takeUnless { it.isBefore(utbetaling.fom()) }
    ...
    private fun Barn.harDatoSomSamsvarer(stønad: TrunkertStønad): Boolean {
        try {
            if (barnetrygdTom()?.isBefore(virkningFom()) == true) {
                // tilhører en feilregistrert stønad
                return false
            }
        ...
        if (stønad.status.toInt() != 0) {
            ...
            if (stønad.antallBarn == 1) {
                ...
                if (gyldigeBeløp.contains(sumUtbetaltBeløp.roundToInt())) {
                    ...
                    secureLogger.info(
                        "Ytelseprosent usikker, ident ${stønad.fnr}, sumUtbetaltBeløp: $sumUtbetaltBeløp, gyldigeBeløp: $gyldigeBeløp" +
    ...
    fun utledListeMedGyldigeUtbetalingsbeløp(
        antallBarn: Int,
        år: Int,
    ): Set<Int> {
    ...
    private fun konverterTilDtoUtvidetBarnetrygd(utvidetBarnetrygdStønader: List<TrunkertStønad>): List<UtvidetBarnetrygdPeriode> {
        ...
        perioder.addAll(
            allePerioder
                .filter { it.stønadstype == SMÅBARNSTILLEGG }
                .groupBy { it.beløp }
                .values
                .flatMap(::slåSammenSammenhengendePerioder),
        )
    ...
    private fun hentValgOgUndervalg(stønad: TrunkertStønad) =
        sakRepository
            .hentBarnetrygdsakerForStønad(stønad)
            .map {
        ...
        perioder
            ...
            .fold(mutableListOf()) { foregåendePerioder, nestePeriode ->
                ...
                if (forrigePeriode?.stønadTom?.isSameOrAfter(månedenFørNestePeriode) == true) {
                    ...
                    when {
                        harSammeFomDato -> {
                            foregåendePerioder.apply { add(maxOf(removeLast(), nestePeriode, compareBy { it.iverksatt })) }
                        }

                        kanSlåesSammen -> {
                            foregåendePerioder.apply { add(removeLast().copy(stønadTom = nestePeriode.stønadTom)) }
                        }

                        else -> {
                            foregåendePerioder.apply {
                                addAll(
                                    listOf(removeLast().copy(stønadTom = månedenFørNestePeriode), nestePeriode),
                                )
                            }
                        }
                    }
    ...
    private fun hentUtvidetBarnetrygdUndervalgFraDb2(stønad: TrunkertStønad) =
        stønad.fnr?.let {
            vedtakRepository
                .hentStønadsklassifisering(
                ...
                ).groupBy { stønadsklasse -> stønadsklasse.vedtakId }
                .values
                .filter { it.kodeNivå2 == "UT" }
                .map { it.kodeNivå3 }
        } ?: emptyList()

    private fun hentBarnetrygdValgOgUndervalgFraDb2(stønad: TrunkertStønad) =
        stønad.fnr?.let {
            vedtakRepository
                .hentStønadsklassifisering(
                    fnr = stønad.fnr.asString,
